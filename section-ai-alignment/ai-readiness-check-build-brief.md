...
**For a fresh Claude Code / Cursor session on Jeanna's laptop. Assume no prior context. 22 July 2026, updated 4 August with the funnel-teardown lessons.**

## Update, 4 August: lessons folded in from the Karla Maree funnel teardown
A parallel thread tore down a $37 personalised-GPT funnel (full record: `jeaniusconsultingproduct_20260804.md`, same folder). Brand decision made: this product ships under vooee, not Jeanius. Five mechanics from that teardown now bind this build:
1. **The structured first-session deliverable.** The report is not "your results"; it is a named, numbered reading (their version: a "12 Part Genius Coaching Session"). Ours: present the report as **"Your AI Leadership Reading, in six parts"** with the parts visibly numbered. A chat-sized output framed as a substantial artefact, honestly.
2. **Personalisation from real inputs, stated proudly.** Their hook is birth details; ours is real business detail plus the perception-gap answers. Say it on the landing page: personalised from what you and your leadership actually think, which is something no generic AI can know.
3. **The emotional anchor is recognition.** Their page sells "finally, someone sees me". The B2B equivalent this product delivers: "someone finally named what is going on in our leadership team." Copy aims for that recognition moment, in vooee's calm register, never manufactured tears.
4. **Demo video of real output.** The strongest asset on their page was a plain screen-share of the product working. Launch checklist addition: Jeanna records a 3-5 minute walkthrough of a real (demo-team) report. No production values needed.
5. **Receipts as proof.** Their proof layer is fake popups and unverifiable counts; ours is the opposite on principle: real completion numbers once they exist, the AIPT pilot story told with permission, and build-in-public documentation of the funnel itself (the marketing of this product is itself a demonstration of vooee's craft). **Hard rules inherited: no fabricated proof, no invented user counts, no fake urgency, refunds handled generously.**
Optional, flagged not decided: their $37 tripwire suggests a possible middle rung later (a low-ticket personal deep-dive between the free check and the $99-149 team snapshot). Do not build it now; the free-to-team ladder ships first.

**Design system note, 4 August:** the vooee visual identity is being re-evaluated (hallmark audit found the cream/serif-italic/coral/eyebrow layer reads as AI-generated; a brand exploration round is pending). Build the quiz's structure and copy now, but treat the design tokens section below as PROVISIONAL: build against CSS custom properties throughout so the winning brand direction can be swapped in by changing the token block only. The hallmark skill is installed in vooee-site2 (branch `claude/add-hallmark-skill`; `.claude/skills/hallmark` for Claude Code, `.cursor/rules/hallmark.mdc` for Cursor) and should be active for all layout work on this build: zero eyebrows, roman headers, and run `hallmark audit` on the finished page before it ships.
